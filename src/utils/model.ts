...
export function generateAndEvaluateCandidates(process: ProcessDefinition): ProcessResult {
  ...
  const baselineResidual = calculatePredictedResidualCu(
    initialCuAtomsCm2,
    baselineRecipe,
    modelParameters,
  );
  ...
  candidatesMap.set(
    `${baselineRecipe.cleaningTimeMin}-${baselineRecipe.rinseTimeMin}-${baselineRecipe.flowRateLpm}-${baselineRecipe.cycles}`,
    baselineCandidate,
  );
  ...
  const middlePass =
    validCandidates.find(
      (c) =>
        ...
        Math.abs(c.upwUsageLiters - (baselineUPW + recommendedCandidate.upwUsageLiters) / 2) < 10,
    ) || validCandidates[Math.floor(validCandidates.length / 2)];

  if (
    middlePass &&
    middlePass.id !== baselineCandidate.id &&
    middlePass.id !== recommendedCandidate.id
  ) {
