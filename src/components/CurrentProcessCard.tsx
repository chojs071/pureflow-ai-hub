...
export const CurrentProcessCard: React.FC<CurrentProcessCardProps> = ({ result }) => {
  ...
  return (
    <div className="rounded-3xl border border-[#E2E8F0] bg-white p-6 sm:p-8 shadow-sm space-y-6">
      ...
      {/* 계산 근거 (Calculation Basis) — 순차 엔진: 초기 → 세정 → 린스 → 게이트, 세정/린스 UPW 분리 */}
      {result.recommendedCandidate.contaminationAfterCleaning !== undefined && (
        <div className="rounded-2xl border border-[#E2E8F0] bg-white p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-center">
            {[
              {
                label: "초기 오염 (C₀)",
                value: formatContaminationValue(
                  result.recommendedCandidate.contaminationAfterCleaning !== undefined
                    ? process.initialContamination
                    : 0,
                  process.qualityMetric.unit,
                ),
              },
              {
                label: `세정 후 (η=${((result.recommendedCandidate.cleaningEfficiency ?? 0) * 100).toFixed(1)}%)`,
                value: formatContaminationValue(
                  result.recommendedCandidate.contaminationAfterCleaning ?? 0,
                  process.qualityMetric.unit,
                ),
              },
              {
                label: `린스 후 최종 (η=${((result.recommendedCandidate.rinseEfficiency ?? 0) * 100).toFixed(1)}%)`,
                value: formatContaminationValue(
                  result.recommendedCandidate.predictedResidual,
                  process.qualityMetric.unit,
                ),
              },
              {
                ...
                value: formatThreshold(
                  result.recommendedCandidate.allowableLimit,
                  process.qualityMetric.unit,
                ),
              },
            ].map((item) => (
              <div
                key={item.label}
                className="rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] px-2 py-2.5"
              >
                <p className="text-[10px] font-bold text-[#64748B] leading-tight">{item.label}</p>
                <p className="font-mono text-[11px] font-bold text-[#071A2E] mt-1">{item.value}</p>
              </div>
            ))}
          </div>
          <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1.5 rounded-xl bg-[#071A2E] px-4 py-2.5 text-xs text-white">
            <span>
              세정 UPW{" "}
              <span className="font-mono font-bold text-[#00C2FF]">
                {result.recommendedCandidate.cleaningUPW ?? 0} L
              </span>
            </span>
            ...
            <span>
              린스 UPW{" "}
              <span className="font-mono font-bold text-[#00C2FF]">
                {result.recommendedCandidate.rinseUPW ?? 0} L
              </span>
            </span>
            <span className="text-[#64748B]">=</span>
            <span>
              ...
              <span className="font-mono font-bold text-white">
                {result.recommendedCandidate.upwUsageLiters} L
              </span>
              {result.batchSize ? ` (배치당, ${result.batchSize}매)` : " (매엽당)"}
            </span>
          </div>
        </div>
      )}
