...
const ANALYSIS_STEPS: StepItem[] = [
  ...
  { id: 5, title: '잔류 오염 계산', description: 'Surrogate Model 기반 예상 잔류 Cu(atoms/cm²) 연산' },
  { id: 6, title: '품질 기준 검증', description: '허용 잔류 기준(≤ Allowable Cu) 초과 후보 자동 탈락' },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4">
      ...
      >
        ...
        <div className="rounded-lg bg-white/5 border border-white/10 p-3 mb-6 flex items-center justify-between text-xs">
          <span className="text-white/70">타겟 공정: <strong className="text-white">{process.subName}</strong></span>
          <span className="text-[#00C2FF] font-mono font-bold">Score {process.contaminationScore}/100</span>
        </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <span className={`text-xs font-bold ${isActive ? 'text-[#00C2FF]' : isCompleted ? 'text-white' : 'text-white/50'}`}>
                      {step.title}
                    </span>
                    {isActive && (
